...
class RolesExport extends DefaultValueBinder implements FromCollection, ShouldAutoSize, WithCustomValueBinder, WithHeadings, WithMapping, WithStyles, WithTitle
{
    /**
    ...
    /**
     * Devuelve la colección de roles a exportar aplicando filtros, ordenación recibida y, finalmente, id descendente.
     */
    public function collection(): Collection
    {
        return Role::query()
            ->byBusqueda($this->busqueda)
            ->byOrdenacion($this->ordenacion)
            ->orderByDesc('id')
            ->get(['name', 'descripcion']);
    }
    ...
    {
        return [
            trans('fields.input.nombre_rol'),
            trans('fields.input.descripcion'),
        ];
    }

    /**
     * Mapea cada rol a la fila de campos que se escribe en el Excel, neutralizando posibles fórmulas en cada celda.
     *
     * @param  Role  $rol
     */
    ...
    {
        return [
            ExcelHelper::sanearFormula($rol->name),
            ExcelHelper::sanearFormula($rol->descripcion),
        ];
    }
    ...
    {
        return trans('fields.roles.titulo');
    }
}
